param(
    ...
    [switch]$History,
    [switch]$Archive,
    [switch]$Select  # New switch for targeted file selection
)
...
if ($History -or $Select) {
    ...
    if ($subDirs.Count -eq 0) {
        ...
        for ($i = 0; $i -lt $subDirs.Count; $i++) {
            ...
            $color = "White"
            # Color scheme based on SLA in folder name
...
$report = $files | ForEach-Object {
    ...
    if ($filename -like "*LOG_DOWN_TCP_S*") {
        ...
        if ($stats) {
            ...
            [PSCustomObject]@{ Time = $_.CreationTime.ToString("MM-dd HH:mm"); RawTime = $_.CreationTime; Type = "DL-TCP-S"; Target = $hostname; Val = $bw; Result = "$bw".PadLeft(5); Max = $contractDown; Visual = "[$(Get-ProgressBar $bw $contractDown)]"; IsError = ($bw -lt ($contractDown * $legalLimit)); FullPath = $_.FullName; Retr = $null }
        }
    } 
    elseif ($filename -like "*LOG_DOWN_TCP_M*") {
        ...
        if ($stats) {
            ...
            [PSCustomObject]@{ Time = $_.CreationTime.ToString("MM-dd HH:mm"); RawTime = $_.CreationTime; Type = "DL-TCP-M"; Target = $hostname; Val = $bw; Result = "$bw".PadLeft(5); Max = $contractDown; Visual = "[$(Get-ProgressBar $bw $contractDown)]"; IsError = ($bw -lt ($contractDown * $legalLimit)); FullPath = $_.FullName; Retr = $retr }
        }
    }
    elseif ($filename -like "*LOG_UP_TCP*") {
        ...
        if ($stats) {
            $bw = [double]$stats.Matches.Groups[1].Value
            [PSCustomObject]@{ Time = $_.CreationTime.ToString("MM-dd HH:mm"); RawTime = $_.CreationTime; Type = "UL-TCP-S"; Target = $hostname; Val = $bw; Result = "$bw".PadLeft(5); Max = $contractUp; Visual = "[$(Get-ProgressBar $bw $contractUp)]"; IsError = ($bw -lt ($contractUp * $legalLimit)); FullPath = $_.FullName; Retr = $null }
        }
    }
    elseif ($filename -like "*LOG_UDP_FRAG*") {
        $stats = $content | Select-String -Pattern "(\d+)/(\d+)\s+\(([\d\.]+)%\)" | Select-Object -Last 1
...
if ($report) {
    ...
    $report | Sort-Object RawTime | ForEach-Object {
        ...
        $displayTarget = if ($_.Target.Length -gt 25) { $_.Target.Substring(0,22) + "..." } else { $_.Target.PadRight(25) }
        $unit = if ($_.Type -match "TCP") { "Mb/s" } else { "" }
        $vs = if ($_.Type -match "TCP") { "vs. $($_.Max) (Min: $($_.Max * $legalLimit))" } else { "Jitter: $($_.Max) ms" }        
        $retrInfo = if ($_.Retr -ne $null) { " | Retr: $($_.Retr)" } else { "" }

        Write-Host ("$($_.Time) | $displayTarget | $($_.Type.PadRight(10)) | ") -NoNewline
        Write-Host ("$($_.Visual) ") -ForegroundColor $color -NoNewline
        Write-Host ("$($_.Result) $unit".PadLeft(12)) -ForegroundColor $color -NoNewline
        Write-Host ("$retrInfo").PadRight(14) -ForegroundColor $(if ($_.Retr -gt 100) {"Red"} elseif ($_.Retr -gt 0) {"Yellow"} else {"Gray"}) -NoNewline
    ...
    if ($medDL) {
        ...
        Write-Host "    Median Download (TCP Single): ".PadRight(45) -NoNewline
        Write-Host ("$([math]::Round($medDL, 2)) Mb/s").PadLeft(20) -ForegroundColor $color
    }
    if ($medDLM) {
        $color = if ($medDLM -lt ($contractDown * $legalLimit)) { $color = "Red" } else { "Green" }
        Write-Host "    Median Download (TCP Multi-10): ".PadRight(45) -NoNewline
        Write-Host ("$([math]::Round($medDLM, 2)) Mb/s").PadLeft(20) -ForegroundColor $color
    }
    if ($medRetr -ne $null) {
        $color = if ($maxRetr -gt 100) { "Red" } else { "Gray" }
        Write-Host "    Retransmits (Median / PEAK): ".PadRight(45) -NoNewline
    ...
    if ($medUL) {
        ...
        Write-Host "    Median Upload (TCP Single): ".PadRight(45) -NoNewline
        Write-Host ("$([math]::Round($medUL, 2)) Mb/s").PadLeft(20) -ForegroundColor $color
    }
    if ($medUDP -ne $null) {
        $color = if ($medUDP -gt 5) { "Red" } else { "Green" }
        Write-Host "    Median Packet Loss (UDP Stress): ".PadRight(45) -NoNewline
    ...
    if ($maxRetr -gt 100) { 
        $diagMessages += "CRITICAL: Excessive TCP Retransmit PEAKS ($([int]$maxRetr)) - Layer 1/2 instability detected."
        $diagColor = "Red"
    }
    if ($medRetr -gt 100) { 
        $diagMessages += "CRITICAL: Excessive TCP Retransmits ($([int]$medRetr)) - Physical Layer failure."
        $diagColor = "Red"
    }
    if ($medUDP -gt 10) {
    ...
    if ($diagColor -eq "Green" -and $medDLM -lt ($contractDown * 0.7)) {
        $diagMessages += "WARNING: Probable Segment Congestion (Downstream Overload)."
        $diagColor = "Yellow"
    }
    ...
    for ($i=0; $i -lt $diagMessages.Count; $i++) {
        if ($i -gt 0) { Write-Host (" " * 25) -NoNewline } # Indentation for subsequent lines
